  ...
  const handleDelete = async () => {
    ...
    try {
      const deleteRequests = selectedData.map(item =>
        axios.delete(`/worklog/${item.id}`)
      );

      await Promise.all(deleteRequests);

      const remainingData = rowData.filter(row => !selectedData.some(selected => selected.employeeId === row.employeeId));
      setRowData(remainingData);
      setSelectedRowCount(0);
    } catch (error) {
      console.error('Error deleting worklogs', error);
      alert('There was an error deleting the selected worklogs.');
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', height: '95vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '90%', marginTop: "0px", marginBottom: '10px', height: '50px' }}>
        <h1 style={{ margin: 0, fontSize: '30px', color: '#172554' }}>Worklogs Details</h1>
        <div>
          <button
            onClick={() => navigate('/worklogs/add')}
            style={{
